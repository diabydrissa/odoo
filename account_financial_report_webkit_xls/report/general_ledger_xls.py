    ...
    def generate_xls_report(self, _p, _xs, data, objects, wb):
        ...
        for account in objects:
            ...
            if _p.display_account_raw(data) == 'all' or \
                ...
                debit_start = rowcol_to_cell(row_start, 7)
                debit_end = rowcol_to_cell(row_pos - 1, 7)
                debit_formula = 'SUM(' + debit_start + ':' + debit_end + ')'
                credit_start = rowcol_to_cell(row_start, 8)
                credit_end = rowcol_to_cell(row_pos - 1, 8)
                credit_formula = 'SUM(' + credit_start + ':' + credit_end + ')'
                balance_debit = rowcol_to_cell(row_pos, 7)
                balance_credit = rowcol_to_cell(row_pos, 8)
