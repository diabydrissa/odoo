    ...
    def generate_xls_report(self, _p, _xs, data, objects, wb):
        ...
        if _p.comparison_mode == 'no_comparison':
            if _p.initial_balance_mode:
                ...
                c_specs += [('init_debit', 1, 0, 'text',
                             _('Initial Debit'), None, cell_style_right)]
                
                c_specs += [('init_credit', 1, 0, 'text',
                             _('Initial Credit'), None, cell_style_right)]
        ...
        if _p.comparison_mode == 'no_comparison' or not _p.fiscalyear:
            ...
            c_specs += [('balance_debit', 1, 0, 'text',
                         _('Balance Debit'), None, cell_style_right)]

            c_specs += [('balance_credit', 1, 0, 'text',
                         _('Balance Credit'), None, cell_style_right)]
        ...
        for current_account in objects:
            ...
            if _p.comparison_mode == 'no_comparison':
                ...
                debit_cell = rowcol_to_cell(row_pos, 6)
                credit_cell = rowcol_to_cell(row_pos, 7)
                bal_formula = debit_cell + '+' + credit_cell
                # cred_bal = debit_cell + '+' + credit_cell
                # deb_bal = debit_cell + '+' + credit_cell

                if _p.initial_balance_mode:
                    init_debit_cell = rowcol_to_cell(row_pos, 3)
                    init_credit_cell = rowcol_to_cell(row_pos, 4)
                    #init_cell = rowcol_to_cell(row_pos, 5)
                    debit_cell = rowcol_to_cell(row_pos, 5)
                    credit_cell = rowcol_to_cell(row_pos, 6)
                    bal_formula = init_debit_cell + '-' + init_credit_cell + '+' +  \
                        credit_cell + '-' + debit_cell 
                    # cred_bal = 'IF(D%i+E%i<0;D%i+E%i;0) %(i)'
                    # deb_bal = 'IF(bal_formula>0;bal_formula;0)' 


                    c_specs += [('init_debit', 1, 0, 'number',
                                 _p['init_debit_accounts'][current_account.id],
                                 None,
                                 cell_style_decimal)]                  


                    c_specs += [('init_credit', 1, 0, 'number',
                                 _p['init_credit_accounts'][current_account.id],
                                 None,
                                 cell_style_decimal)]
                ...
                if _p['balance_accounts'][current_account.id] < 0:

                    cred_bal = init_credit_cell + '-' + init_debit_cell + '-' +  \
                        debit_cell + '+' + credit_cell
                else:

                    deb_bal = init_debit_cell + '-' + init_credit_cell + '+' +  \
                        debit_cell + '-' + credit_cell 
               
                
                c_specs += [('balance_debit', 1, 0, 'number', None, 
                             deb_bal, cell_style_decimal)]

                c_specs += [('balance_credit', 1, 0, 'number', None, 
                             cred_bal, cell_style_decimal)]
